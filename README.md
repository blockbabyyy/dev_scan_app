...
Результаты по типам файлов, где движки расходятся. Курсивом — отклонение от Hyperscan.

| Тип | Цель | Hyperscan | RE2 | Boost |
|-----|------|----------:|----:|------:|
| JPG | `001.zip` | **46** | 46 | **100** *(+54)* |
| HTML | `001.zip` | **42** | **36** *(−6)* | 42 |
| JSON | `001.zip` | **11** | **9** *(−2)* | 13 *(+2)* |
| JPG | `280.zip` | **10** | 10 | **44** *(+34)* |
| HTML | `280.zip` | **94** | **74** *(−20)* | 94 |
| GZIP | `280.zip` | **10** | 10 | **13** *(+3)* |
| PE | `280.zip` | **0** | 0 | **3** *(+3 FP)* |
| JPG | `2026/` | **2** | 2 | **3** *(+1)* |
| EMAIL,GIF,JPG,PNG,HTML,JSON | `001.pcap` | совпадают | совпадают | совпадают |

Hyperscan и RE2 полностью совпадают на `001.pcap`. Совпадение подтверждено Python-эталоном `ref_scan.py`.
...
Проблема зафиксирована, исправление запланировано (требует передачи `match_not_bol`
после первого совпадения и явного флага `no_mod_newline` при компиляции паттерна).

#### RE2 — недобор HTML / JSON
...
Если при компиляции `RE2::Set::Add()` один из паттернов не принят, индексы
`matched_ids` сдвигаются относительно вектора регулярных выражений второй фазы.
Для отдельных файлов применяется неверный паттерн второй фазы, что даёт заниженный счёт.

Проблема зафиксирована (к задаче GPT P1-2 в to-do.txt), исправление запланировано.

#### PCAP — все движки дают одинаковый результат

При обработке `.pcap`-файлов с флагом `-e hs` движок автоматически заменяется на RE2.
Причина: Hyperscan в block-mode сообщает о каждой позиции конца совпадения (end-of-match),
а не о непересекающихся вхождениях. Для паттернов вида `GIF8.*?;` это давало тысячи
ложных срабатываний вместо реальных ~1186.
RE2 использует `FindAndConsume` с корректной семантикой непересекающихся совпадений.
Boost для PCAP работает «как есть» (RE2 не подставляется), но показывает верный результат
при условии отсутствия бинарных артефактов в потоке.
