...
// Хелперы для подсчета количества совпадений
...
// Быстрая предварительная проверка наличия сигнатур
// [FIX] Если заголовка нет, или хвоста нет - выходим сразу, не дергаем regex 
// ( Пока не используется, но без него бесконечно ждать std и Boost)
...
void StdScanner::scan(const char* d, size_t s, ScanStats& st) {
    ...
    auto run_check = [&](const std::string& label, const std::regex& re, int& counter, const std::string& sig_check = "") {
        //Быстрая проверка сигнатуры (Pre-filter)
        if (!sig_check.empty() && !has_sig(d, s, sig_check)) {
            return; // Пропускаем, если нет сигнатуры
        }

        // Вывод статуса (перезаписываемая строка, чтобы првоерить не подвис ли)
...
void HsScanner::scan(const char* data, size_t size, ScanStats& stats) {
    ...
    auto on_match = [](unsigned int id, unsigned long long from, unsigned long long to, unsigned int flags, void* ptr) -> int {
        ...
        // [FIX] Защита от дребезга (Debounce)
        // Hyperscan находит каждое вхождение. Если файл разбит на чанки или в нем много повторов,
        // мы можем засчитать его 100 раз.
        // Если новое совпадение слишком близко к старому (< 512 байт) - игнорим.
        if (id < 32) {
